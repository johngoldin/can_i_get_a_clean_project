...
suppressPackageStartupMessages(library(scico))       ## scico color palettes(http://www.fabiocrameri.ch/colourmaps.php) in R 
suppressPackageStartupMessages(library(ggtext))      ## add improved text rendering to ggplot2
suppressPackageStartupMessages(library(ggforce))     ## add missing functionality to ggplot2
suppressPackageStartupMessages(library(ggdist))      ## add uncertainity visualizations to ggplot2
# library(magick)      ## load images into R
suppressPackageStartupMessages(library(patchwork))   ## combine outputs from ggplot2
# library(Cairo)
...
This is a follow-up to an [earlier post](https://www.johngoldin.com/blog/2020-02-apple-health-export-1/) 
...
walk in the woods almost every day with an occasional longer walk thrown in
...
VO2 Max. My conclusion is that I should not attribute short-run changes in
...
a stable methodology. I look forward to seeing that someday. 
...
In September 2021, after I first published this post,
a web search led me to a
[technical article](https://www.apple.com/healthcare/docs/site/Using_Apple_Watch_to_Estimate_Cardio_Fitness_with_VO2_max.pdf)
that provides a great deal of detail about how
Apple created their estimate of VO2 Max. The article is posted
in the [healthcare](https://www.apple.com/healthcare/) section of their
web site, although I only found it as a result of a web search. I
don't see where it is listed at the healthcare site.
In any case, I give credit for creating some substantial background
information rather than simply asking us to accept their measures on 
faith.
...
They study describes the study used to create the estimates of VO2 Max introduced with
OS7.
I was happy to see that 38% of the participants is the study were over 65.
They report that the correlation between their estimate of VO2 Max
with a direct measure
via cardiopulmonary exercise testing
is 0.89 for men and 0.86 for women.


## Appendix: Some of the R Code Used to Make This Post

In my [earlier post](https://www.johngoldin.com/blog/2020-02-apple-health-export-1/)

...
In this post I will dig into why CDC and CT Department of Public Health
(CT DPH) are reporting different COVID-19 vaccine rates for Connecticut.
The goal is to understand how vaccinations are counted and how data
makes its way into the CDC summaries. Connecticut DPH is reporting
tabulations based on [CT
WiZ](https://portal.ct.gov/DPH/Immunizations/ALL-ABOUT-CT-WiZ), the
Connecticut immunization information system.

The table below shows the Connecticut COVID-19 vaccination of Connectiut
residents reported by the CDC and by the Connecticut Department of
Public Health as of
`r  max(both_sources_state_total$cdc_date, both_sources_state_total$date)`.
...
The major reason that the CT DPH count are lower overall is that DPH
...
required](https://portal.ct.gov/-/media/Coronavirus/20201207-DPH-Order-regarding-reporting-COVID-19-vaccinations.pdf)
to report vaccinations to CT WiZ within 24 hours. I received my first
...
need to communicate that to them separately.)
...
different jurisdictions or at different providers because of the need to
remove personally identifiable information (de-identify) data to protect
people's privacy," according to a footnote on the CDC's covid vaccine
data tracker webpage. "This means that, even with the high-quality data
CDC receives from jurisdictions and federal entities, there are limits
to how CDC can analyze those data."

This [Kaiser Health
News](https://khn.org/news/article/cdc-senior-covid-vaccination-rates-appear-inflated/)
article suggested that CDC vaccination rates may be too high and cited
the same issues cited in the CDC footnote.
...
First I'll display a direct comparison of county by county vaccination
data for Connecticut reported by the CDC and the tallies from the
...
with an overview of the IT systems for tracking vaccinations.
...
The basic source they rely on is the Connec \#\# On the General Issue of
the Role of Data in Responding to the Pandemic

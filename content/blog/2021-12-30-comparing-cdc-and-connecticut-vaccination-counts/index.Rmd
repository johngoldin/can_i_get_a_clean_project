...
Here are the current Connecticut totals from the CDC and from the
...
Note that CDC reports 14% more individuals with some vaccination,
including 54% more who have received only the initial dose.

This post will to into more detail to compare COVID vaccination rates
reported by Connecticut Department of Public Health with the reports
from the CDC. The goal is to understand how vaccinations are counted and
how data makes its way into the CDC summaries.

Below are two comparisons county-level data from the CDC to DPH, first
with a table and then with a bar chart. Until recently "fully
vaccinated" referred to individuals with two doses of Pfizer or Moderna
or one dose of J&J. In this post we'll refer to those people as "two
does" even if they have had only one dose of J&J.

```{r print-table}
for_table
```

Below is a visual summary of the same vaccination counts by source. The
brown bars show vaccinations reported by CDC and the green bars show
Connecticut Department of Health. The figure includes a data point to
show the total population of each county according to the 2020 Census.
...
This post will compare vaccination counts (and vaccination rates)
published by the CDC and by the Connecticut Department of Public Health.

First, let's look at a summary of county-by-county COVID vaccinations in
Connecticut reported by CDC and by CT DPH. After that we will dig into
why these counts are different.

```{r county_summaries_by_source, echo = FALSE}

```

For Connecticut, the CDC reports about 8% more individuals who are fully
vaccinated than does DPH. Looking at partially vaccinated (one dose of
Moderna or Pfizer), CDC reports 46% more individuals. Counting things
can be hard. The details of what is included and how the counts are done
makes a difference.

This pattern has been observed in other states. There are a number of
attempts to go into the details of CDC versus state counts for
particular states. See this [recent analysis of county level counts in
...
See also comments by Washington State on [differences between their
counts and
...
The major reason that the CT DPH count are lower is that they do not
include vaccinations by "federal sources" such as the VA or DOD.

> ...does not included doses administered to CT residents by
> out-of-state providers or by some Federal entities (including
> Department of Defense, Department of Correction, Department of
> Veteran's Affairs, Indian Health Service) because they are not yet
> reported to CT WiZ (the CT immunization Information System)

States may vary as to whether they include data from federal providers
and whether they have information about vaccination of residents that
were administered in other states. In neighboring Massachusetts, their
[vaccination
report](https://www.mass.gov/doc/daily-covid-19-vaccine-report-january-21-2022/download)
states that their numbers only included vaccinations reported from the
Veterans Administration for MA residents as of 11/4/2021. State
immunization information systems may not have information from all
providers in the state.

How comprehensive is the information for Connecticut? I don't really
know. I received my first two doses from a clinic operated by Yale New
Haven Hospital. My booster dose came from Walgreens. When I looked up my
vaccination record in CT WiZ I was pleased to see that all three doses
were recorded (as were my flu shots for 2020 and 2021). I receive my
primary care through a system that uses the Yale New Haven Hospital
electronic health record, but in that system my third dose of COVID
vaccination is *not* recorded. (I'll need to communicate that to them
separately.)

Another problem for the state totals is the possibility of vaccination
done out of state. This may be an issue for college students and
"snowbird" retirees who spend part of the year in different states.
Also, the state of residence listed at the time of a vaccination for
such individuals may not be the same place the individual is counted in
the 2020 census. Uncertainty about residence is an issue for both the
numerator and denominator of reported vaccination rates by locality.

This [Kaiser Health
News](https://khn.org/news/article/cdc-senior-covid-vaccination-rates-appear-inflated/)
article suggested that CDC vaccination rates may be too high.
...
First I'll do a direct comparison of county by county vaccination data
for Connecticut reported by the CDC and the tallies from the Connecticut
[Department of Public
Health](https://data.ct.gov/stories/s/CoVP-COVID-Vaccine-Distribution-Data/bhcd-4mnv/).
Then I will dig into the known differences between these reports. Both
the CDC and CT DPH have made a real effort to be transparent about their
reports. There's quite a bit of information available on the Web
describing how they are tabulating vaccinations.

### Connecticut Department of Public Health Vaccination Reporting

This post will focus on my home state of Connecticut. DPH reports weekly
on [vaccinations administered through Connecticut
providers](https://data.ct.gov/stories/s/CoVP-COVID-Vaccine-Distribution-Data/bhcd-4mnv/).
The basic source they rely on is the Connec
...
about tracking the COVID vaccines.

### Difficulties in Counting "Fully" Vaccinated

The CDC gives a [full
discussion](https://www.cdc.gov/coronavirus/2019-ncov/vaccines/reporting-vaccinations.html)
reasons why it may be hard for them to track different doses of COVID
vaccines at the national level. That's especially true for boosters
because there is a long internval after the second dose and a booster is
...
CDC provides an [overview of IT systems for tracking COVID
vaccinations](https://www.cdc.gov/vaccines/covid-19/reporting/overview/IT-systems.html).
The information architecture depends on 64 Immunization Information
...
always covered other immunizations as well. That's why I can see my many
of my non-COVID vaccinations in my Connecticut IIS (CT WiZ).

They describe a lot of detail about the legal process.

It looks like there are supposed to be [explicit
provision](https://www.cdc.gov/vaccines/covid-19/reporting/iz-gateway/gateway-qa.html)
...
## On the General Issue of the Role of Data in Responding to the Pandemic
...
> does their protection $$wane?$$. Those people who are getting
> hospitalized who had a third shot, what's going on there? We were
> promised by the CDC in May that they would track every breakthrough
> hospitalization and death. Never happened. And so we don't have
> critical data. We have 150,000-plus people in the hospital. And I have
> begged at the HHS level that we have granular data on each of those
> people. Their vaccination status, which vaccine, when they got it,
> their age, their co-existing conditions, and all the basal stuff,
> right? And they say, "Well, we're gonna look into it." Even though it
> can be mandated by HHS authority. So we have just a dreadful situation
> here where we don't have real-time capture of data. The only data that
> gets posted on the CDC is a month old, and it's de minimus about
> things like hospitalizations or deaths, and this is unacceptable. If
> you're gonna go in a pandemic and try to deliver and guidance, you
> have to have data. That's how you develop trust. That's how you have
> the best navigation system. And we don't have it. But the most
> important thing to me, Bob, is there's no seeming will to get one. And
> that is distressing and it has to change.
>
...
pandemic in the US? Possibly not. But individuals and local governments

...
#| label: census_ctdph
#| cache: true
# This chunk will get the ACS5 2020 pop by age range by town
# Use this to convert from age ranges in ACS to age range in CT DPH vaccination data.
# 
convert_acs_to_ctdph = tribble(   
  ~acs,            ~ctdph1,       ~age,
 '5 to 9 years', "young", "5-17",     
 '10 to 14 years', "young",      "5-17",  
 '15 to 17 years', "young",      "5-17",
 '18 and 19 years', "population_18-24",  "18-44", 
 '20 years', "population_18-24",  "18-44",        
 '21 years', "population_18-24",  "18-44",        
 '22 to 24 years' , "population_18-24",  "18-44", 
 '25 to 29 years', "population_25-44",  "18-44",  
 '30 to 34 years', "population_25-44",  "18-44",  
 '35 to 39 years', "population_25-44",  "18-44",  
 '40 to 44 years', "population_25-44",  "18-44",  
 '45 to 49 years', "population_45-64", "45-64",   
 '50 to 54 years', "population_45-64", "45-64",   
 '55 to 59 years', "population_45-64", "45-64",   
 '60 and 61 years', "population_45-64", "45-64",  
 '62 to 64 years', "population_45-64", "45-64",   
 '65 and 66 years', "population_65+", "65+",  
 '67 to 69 years', "population_65+", "65+",   
 '70 to 74 years' , "population_65+", "65+",  
 '75 to 79 years', "population_65+", "65+",   
 '80 to 84 years', "population_65+", "65+",   
 '85 years and over', "population_65+", "65+",
 'Total', "total",    "total",        
 'Total Female', "total female",   "total female",    
 'Total Male' , "male",    "male",   
 'Under 5 years', "young",      "<5",)
census_pop_base <- get_acs_age_sex(2020, acs_vars = load_variables(2020, "acs5", cache = TRUE))
acs_age_range <- census_pop_base |> 
  group_by(town = TOWN, census_age) |> 
  summarise(acs_pop = estimate) |> 
   left_join(convert_acs_to_ctdph, by = c("census_age" = "acs"))
# version #2 has ctdph2 as <18, 18-44, 45-54, 55-64, 65+ 
census_pop2 <- acs_age_range |> 
  mutate(measure = "acspop") |> 
  filter(age != "male", age != "female", age!= "total female", age != "total") |> 
  group_by(town, measure, age) |> 
  summarise(value = sum(acs_pop), .groups = "drop") 
```
...
# dph vaccination data is imported from 
# that is created by daily_ct_stats.R.
# To exaine state data of town  by town vaccinations, 
# go to https://data.ct.gov/resource/gngw-ukpw
...
# vote_vaccinated3 <- vote_vaccinated |> ungroup() |> 
#   select(town, pct_repub,
#          social_vulnerability_index, 
#          `<18` = under_18_vaxed, `18-44` = young_18_to_44_vaxed, 
#          `45-64` = middle_vaxed, `65+` = `old_65+_vaxed`) |> 
#   pivot_longer(names_to = "Age Group",
#                cols = c(`<18`, `18-44`, `45-64`, `65+`)) |> 
#   left_join(census_pop2, by = c(""))
# vac_count_age assembles raw vax count by four age groups so that
# later we can apply ACS5 2020 data as denominator for vax rate.
vax_count_age1 <- vote_vaccinated |> ungroup() |> 
    mutate(`vax_5-17` = (`fully_vaccinated_5-11` + `fully_vaccinated_12-17`) ,
         `vax_18-44` = (`fully_vaccinated_18-24` + `fully_vaccinated_25-44`),
         `vax_45-64` = `fully_vaccinated_45-64` ,
         `vax_65+` = `fully_vaccinated_65+`,
         `population_5-17` = `population_5-11` + `population_12-17`,
         `population_18-44` = (`population_18-24` + `population_25-44`),
         `dphrate_5-17` = `vax_5-17` / `population_5-17`,
         `dphrate_18-44` = `vax_18-44` / `population_18-44`,
         `dphrate_45-64` = `vax_45-64` / `population_45-64`,
         `dphrate_65+` = `vax_65+` / `population_65+`
         ) |> 
  select(town, # pct_repub, social_vulnerability_index, pop2020,
         starts_with("vax"), `dphpop_5-17` = `population_5-17`, 
         `dphpop_18-44` = `population_18-44`, 
         `dphpop_45-64` = `population_45-64`, `dphpop_65+` = `population_65+`, 
         starts_with("dphrate")) |> 
  pivot_longer(names_to = c("measure", "age"), values_to = "value",
               cols = c(`vax_5-17`:`dphrate_65+`), names_sep = "_")
vax_count_age <- vax_count_age1 |> 
  bind_rows(census_pop2)
vax_age_wide <- vax_count_age |>
  pivot_wider(id_cols = town,  values_from = value,
              names_from = c(measure, age),
               values_fill = 0) |> 
  # join town variables back in
  left_join(vote_vaccinated |> ungroup() |> 
    select(town, social_vulnerability_index, pop2020, pct_repub, dateupdated) |> 
    unique(), by = "town") |> 
  mutate(`acsrate_5-17` = `vax_5-17` / `acspop_5-17`,
         `acsrate_18-44` = `vax_18-44` / `acspop_18-44`,
         `acsrate_45-64` = `vax_45-64` / `acspop_45-64`,
         `acsrate_65+` = `vax_65+` / `acspop_65+`,
         acs_total_same = `acspop_5-17` + `acspop_18-44` + `acspop_45-64` + `acspop_65+`,
         acs_total = acs_total_same + `acspop_<5`,
         dph_total = `dphpop_5-17` + `dphpop_18-44` + `dphpop_45-64` + `dphpop_65+`)
for_checkup <- vax_age_wide |> 
  mutate(total_dif = abs(acs_total - dph_total) / dph_total,
         old_rate_dif = abs(`acsrate_65+` - `dphrate_65+`)) |> 
  relocate(town, pop2020, acs_total, acs_total_same, dph_total, social_vulnerability_index,
          `acspop_5-17`, `dphpop_5-17`,`acspop_18-44`, `dphpop_18-44`, 
          `acspop_45-64`, `dphpop_45-64`, `acspop_65+`, `dphpop_65+` )
  
# Used this to compare decenial census with ACS2020 and DPH estimate of 2020       
# p_compare_acs_pop <- ggplot(data = for_checkup, aes(x = `acs_total`, y = `pop2020`)) + 
#   geom_abline() + 
#   geom_point(aes(size = pop2020), colour = "grey30") +
...
#   geom_text(aes(label = town), size = 2.5, nudge_y = -0.005) +
#   ylab("Population (Decenial)") + xlab("Population (ACS2000 estimate)") +
#   theme(legend.position="none") +
#   theme(plot.caption = element_text(hjust = 0)) +
#   labs(title = "Comparison of Population Estimate Based on ACS or DPH",
#        caption = "")

# Next use pivot_longer to get one separate age range within town
# rather than one row per town.
vax_town_age_acs <- vax_age_wide |>
  select(town, pop2020, social_vulnerability_index, pct_repub, dateupdated,
         `acsrate_18-44`, `acsrate_45-64`, `acsrate_5-17`,`acsrate_65+`) |> 
  pivot_longer(cols = starts_with("acsrate"), 
               names_to = "Age Group") |> 
  mutate(`Age Group` = str_replace(`Age Group`, "acsrate_", "")) |> 
  left_join(  # do a second pivot_longer to also get population that goes with rate
    vax_age_wide |> 
      select(town, dateupdated,
             `acspop_18-44`, `acspop_45-64`, `acspop_5-17`,`acspop_65+`) |> 
      pivot_longer(cols = starts_with("acspop"), 
                   names_to = "Age Group", values_to = "pop") |> 
      mutate(`Age Group` = str_replace(`Age Group`, "acspop_", "")), 
    by = c("town", "dateupdated", "Age Group")
  ) |> 
  mutate(`Age Group` = factor(`Age Group`, 
                              levels = c("5-17", "18-44", "45-64", "65+")),
         flag_for_removal = case_when(
           (town == "Mansfield") & (`Age Group` %in% c("5-17", "18-44")) ~ TRUE,
           town == "Simsbury" ~ TRUE,
           TRUE ~ FALSE
         ))

vax_town_age_dph <- vax_age_wide |>
  select(town, pop2020, social_vulnerability_index, pct_repub, dateupdated,
         `dphrate_18-44`, `dphrate_45-64`, `dphrate_5-17`,`dphrate_65+`) |> 
  pivot_longer(cols = starts_with("dphrate"), 
               names_to = "Age Group") |> 
  mutate(`Age Group` = str_replace(`Age Group`, "dphrate_", "")) |> 
  left_join(  # do a second pivot_longer to also get population that goes with rate
    vax_age_wide |> 
      select(town, dateupdated,
             `dphpop_18-44`, `dphpop_45-64`, `dphpop_5-17`,`dphpop_65+`) |> 
      pivot_longer(cols = starts_with("dphpop"), 
                   names_to = "Age Group", values_to = "pop") |> 
      mutate(`Age Group` = str_replace(`Age Group`, "dphpop_", "")), 
    by = c("town", "dateupdated", "Age Group")
  ) |> 
  mutate(`Age Group` = factor(`Age Group`, 
                              levels = c("5-17", "18-44", "45-64", "65+")),
...
vax_town_age <- vax_town_age_acs |> 
  left_join(
    vax_town_age_dph |> 
      select(town, `Age Group`, dateupdated, value, pop),
    by = c("town", "Age Group", "dateupdated"),
    suffix =c("_acs", "_dph")
  )
...
p <- ggplot(data = vax_town_age |> filter(!flag_for_removal), 
            aes(x = pct_repub, y = value, colour = `Age Group`)) +
              geom_point()
# remove Mansfield because of Storrs
p_vote_vaccine_acs <- ggplot(data = vax_town_age |> filter(!flag_for_removal), 
                         aes(x = pct_repub, y = value_acs,
                             colour = `Age Group`)) +
  geom_smooth(data = vax_town_age |> filter(social_vulnerability_index != "Yes",
                                             !flag_for_removal),
              aes(colour = `Age Group`),
              method = "lm") +
  geom_point(aes(shape = social_vulnerability_index)) +
  geom_text(aes(label = town), size = 2.5, nudge_y = -0.005) +
  ylab("Fully Vaccinated") + xlab("Trump vote in 2020") +
  scale_y_continuous(labels=percent) + scale_x_continuous(labels=percent)

p_vote_vaccine_dph2 <- ggplot(data = vax_town_age |> filter(!flag_for_removal), 
                         aes(x = pct_repub, y = value_dph,
                             colour = `Age Group`)) +
  geom_smooth(data = vax_town_age |> filter(social_vulnerability_index != "Yes",
                                             !flag_for_removal),
              aes(colour = `Age Group`),
...
# Manfield and Canaan are extreme outliers, Mansfield because of
# UConn but I don't know explanation for Canaan.
 p_pops1 <- ggplot(data = vote_vaccinated2 |> 
   ...
   theme(legend.position="none") +
   theme(plot.caption = element_text(hjust = 0)) +
   labs(title = "Trump Vote in 2020 and COVID Vaccination Rate in Connecticut Towns",
        caption = "\n'fully vaccinated' means one dose of J&J or two of Pfizer or Moderna")
...
#   mutate(social_vulnerability_index = factor(social_vulnerability_index,
#                                              levels = c("No", "Yes"), 
#                                              ordered = TRUE))
p_pops2 <- ggplot(data = vote_vaccinated2 |> 
 ...
 # geom_smooth(aes(size = pop2020, y = vax_rate2020_over17), colour = "grey", method = "lm") +
   ...
   scale_size_area(breaks = c(1000, 2000, 5000, 10000, 30000, 100000),
                   guide = "none") +
   geom_text(aes(label = town), size = 2.5, nudge_y = -0.005) +
   labs(title = "Trump Vote and Vaccination Rate") +
   guides(colour = guide_legend("Social Vulnerability"))
...
# I don't plan on using p_pops1b_with_hist, just wanted to see what it looked like.
p_pops1b_with_hist <- p_pops1b + geom_text(data = lm_lines |> filter(social_vulnerability_index == "No"),
...
```{r}
#| label: fig-vote-repub
#| message: false
#| fig-cap: Trump Vote in 2020 and COVID Vaccination Rate
...
```{r}
#| label: fig-social-vulnerability
#| message: false
#| fig-cap: Considering Social Vulnerability of Town
...
#| label: fig-age
#| message: false
#| fig-cap: Separately by Age of Vaccinated

print(p_vote_vaccine_acs)
```
...
if (1 == 2) {
  ...
  sex_age_acs2014_3 <- sex_age_acs2014_2 |> 
    group_by(TOWN, age) |> 
    summarise(estimate = sum(estimate))
  sex_age_acs2020_2 <- get_acs_age_sex(acs_year = 2020, acs_vars = vars)
...
# in effect I'm making my own more general cache for the census data
...
sex_age_acs2020_2 |> 
  group_by(age) |> 
  summarise(estimate = sum(estimate)) |> 
  mutate(pct = estimate /  sum((sex_age_acs2020_2$estimate[sex_age_acs2020_2$age == "Total"])), estimate = comma(estimate), pct = percent(pct))

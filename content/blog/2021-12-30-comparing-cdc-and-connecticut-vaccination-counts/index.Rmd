...
cdc_asof <- format(max(three_source_state_total$cdc_date, na.rm = TRUE), "%d-%b-%Y")
...
for_table <- three_sources |> ungroup() |> 
  ...
  tab_spanner(label = "'fully' vaccinated",
...
Department of Public Health with the reports from the CDC. The table
below shows the most recent counts. In this table "fully vaccinated"
means either two doses of Pfizer or Moderna or one does of J&J. The may
be changing. In any case, the CDC reports about 8% more individuals who
are fully vaccinated than does DPH. Looking at partially vaccinated (one
dose of Moderna or Pfizer), CDC reports 46% more individuals.
...
Below is a visual summary of the vaccination counts by source. The brown
bars show vaccinations reported by CDC and the green bars show
Connecticut Department of Health. As elsewhere in this post, "full"
vaccination means two shots of Moderna or Pfizer or one of J&J. It's
possible that the CDC definitioin of "fully vaccinated" will change in
the future. The figure includes a data point to show the total poplation
of each county according to the 2020 Census.
...
Often there are differences between COVID vaccination rates reported by
the CDC and rates produced by individual state health agencies. The
counts don't agree largely because there are important differences in
what they are including in the counts. Counting things can be hard, and
the details of how the counts are done makes a difference.

First, let's look at a summary of county-by-county COVID vaccinations in
Connecticut reported by CDC and by CT DPH. Then we will dig into why
...
> [&#x0040;ONS](https://twitter.com/ONS), whose random sample survey
> providing regular and representative estimates of prevalence is truly

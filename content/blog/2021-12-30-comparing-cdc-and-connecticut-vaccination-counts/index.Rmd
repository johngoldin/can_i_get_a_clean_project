...
Officer](https://www.hhs.gov/cto/index.html) at HHS, although the
...
Health Laboratories](https://www.aphl.org) (APHL), presumably under a
contract from HHS. Unfortunately the APHL web site offers no information
about the gateway.

What is actually happening with the gateway?

The diagram shows a lot of two-way arrows, but the two directions are
not equal. For participating states, all of the assembled by the state
IIS flows into the IZ Gateway. Data does not flow so freely back down to
the state. It seems that data for Connecticut residents who get their
vaccinations out of state does not automatically flow into CT WiZ (the
Connecticut IIS). One must also keep in mind that individuals may list a
local residence when they are vaccinated. For states that have signed
the relevant memorandum of understanding, they can retrieve all of the
immunizations for that individual from the IZ Gateway. In that way,
Connecticut may be able to check whether a particular person has gotten
vaccinations in another state. I'm not sure whether that actually
happens. The effect is that for the most part data goes one-way from the
state IIS to the central IZ Gateway. As a result, states IIS's do not
learn about out of state vaccinations of their residents[^1].
Vaccinations by federal agencies are recorded via VAMS. That goes into
to the IZ Gateway but does not seem to be passed on to the state IIS's.
The planning diagram has lots of multi-way arrows, but in practice the
arrows seem to go one-way into the IZ Gateway and dead end there. That
limits the comprehensiveness of the reporting done by state agencies
such as the CT Department of Public Health. While the IZ Gateway was
conceived before COVID, the pandemic increased the urgency. An article
"[How the US plans to track Covid-19 vaccine
...
[^1]: Some states make bilateral arrangments with particular neighboring
    states. For example, Wisconsin gets data from Minnesota and from
    Michigan.
...
on the CDC site seems to say:[^2]

[^2]: Although the Q&A page says it was last reviewed in August 2021, I
...
Providing the federal vaccination data to the states seems like a basic
service that the IZ Gateway should provide, but it doesn't seem to be
happening.

The legal underpinning for exchanging data among states and with the CDC
depends on a complicated set of state-by-state legal agreements.

...
both_sources_state_total <- both_sources_detail |> 
  ...
  summarise(across(c(cdc_fully_vaccinated:cdc_init_only) & 
                     ...
                     sum, na.rm = TRUE)) |> 
  mutate(state = "CT", county = "Total CT", date = both_sources_detail$date[[1]],
...
This post will compare COVID vaccination rates reported by Connecticut
Department of Public Health with the reports from the CDC. The goal is 
to understand how vaccinations are counted and how data makes
its way into the CDC summaries.

Below are two comparisons of CDC to DPH,
first with a table and then with a bar chart.
Until recently "fully vaccinated" referred to individuals with two doses
of Pfizer or Moderna or one dose of J&J. In this post we'll refer to
those people as "two does" even if they have had only one dose of J&J.
...
both_sources_simple <- both_sources |> 
  ...
  pivot_longer(cols = dph_county_fully:dph_additional_dose_received) |> 
  mutate(source = ifelse(str_detect(name, "dph"), "CT DPH", "CDC") |> 
           factor(levels = c("CDC", "CT DPH")),
         status = ifelse(str_detect(name, "fully"), "Full", 
                         ifelse(str_detect(name, "init"), 
                                "Initial", "Booster")) |> 
           factor(levels = c("Initial", "Full", "Booster")),
         county = factor(county),
         grouping = paste0(source,"-", status) |> 
           factor(levels = c("CDC-Initial", "CDC-Full", "CDC-Booster", "CT DPH-Initial", "CT DPH-Full", "CT DPH-Booster")))

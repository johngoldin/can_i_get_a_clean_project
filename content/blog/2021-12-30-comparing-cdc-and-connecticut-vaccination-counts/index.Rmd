...
DPH) are reporting different COVID-19 vaccination counts for Connecticut. The
goal is to understand how the data makes
...
State-wide and county by county the pattern is that CDC reports more people
vaccinated with at least one dose and more people who have received only the first
...
location. The de-identified data that CDC analysts work with does not include the
...
In December 2021 there were a number of articles that discussed
issues with the quality of the CDC vaccination tabulations
(for example, [Matt Yglesias](https://www.slowboring.com/p/the-cdcs-vaccine-data-is-all-wrong?s=w)).
...
that system my third and fourth doses of COVID vaccination are *not* recorded. (I'll
...
residence reported at the time of vaccination may
not be the same residence reported to the Census tabulations which drives 
the denominator of the rate calculation.
College students, "snowbird" retirees who
spend part of the year in different states, or migrant workers
may not be counted consistently in the
numerator and the denominator. When someone gets a vaccination they are most
likely to give their location at the time
of the vaccination as their residence. That may not be the same place the
individual is counted in the 2020 census. 

For example, the town of Mansfield is the home of the 
University of Connecticut. It has the lowest reported vaccination
rate of any town in Connecticut. In the Census, students who live on
campus are counted as residents of Mansfield and are in the denominator
of the rate calculation. At the time they were vaccinated many of those
students may have reported their home address as the address where
they live with their families when school is not in session.
Younger populations
are less likely to be vaccinated, but a large part of what produces
the reported low vaccination rate in Mansfield is 
that "home address" in the vaccination data often does
not match where the student was counted by the Census Bureau.
...
Internet searching of the CDC web site turns up a lot of
information about CDC tracking COVID vaccinations, but almost all of it
refers to planning done during late 2020 in preparatioin for the roll
out of vaccines. CDC has a [web
...
*Keep in
mind that this chart and other planning documents represent a goal 
and cannot be taken as a description
...
[explicit legal agreemenets](https://www.cdc.gov/vaccines/covid-19/reporting/iz-gateway/gateway-qa.html)
...
The theory is that the IZ Gateway is a central hub through
which data can move back and forth among the various
state-based IIS's.

What is actually happening?
My impression is that for the most part the agreements have not
been made to exchange this data and that data mostly goes
one-way from the state IIS to the central IZ Gateway. As a result,
states IIS's do not learn about out of state vaccinations of their
residents. Vaccinations by federal agencies are recorded via
VAMS. That goes into to the IZ Gateway but does not seem to be
passed on the state IIS's. 

The planning diagram shown above has lots of multi-way arrows,
but in practice the arrows seem to go on way into the IZ Gateway
and dead end there. That limits the comprehensiveness of the
reporting done by state agencies such as the CT Department of Public Health.

<a href="https://www.aphl.org/programs/informatics/Pages/COVID-19-resources.aspx">
<img class="ms-rtePosition-2" alt="COVID-19 Data Flow on AIMS Platform" src="https://www.aphl.org/PublishingImages/INFO-2020-AIMS-COVID-19-Data-Flow-Infographic.jpg" style="margin&#58;10px 15px;width&#58;495px;height&#58;387px;"/>
</a>

#### Is "Privacy Theater" Hobbling the Data?

It appears that one major problem for the CDC analysts is
that they do not have access to all the identifying information
that is reported in from the state IIS's. That makes it hard
to identify whether a particular shot is a first shot or
a follow-on shot.

There is an [HHS Privacy Assessment](https://www.hhs.gov/sites/default/files/cdc-ncird-immunization-datalake-and-data-storefront-izdl.pdf)
...
There's exists a central national database that contains
individually identified inoculation data. But that is
not the database used by CDC analysts to tabulate
that vaccination rates by state and county.
Before tabulation, there is an additional step
that creates another database with most individual
